...
{
	glm::vec3 point1 = TransVector(model.GetVertice(face.GetVertexIndex(1) - 1), model, camera) - TransVector(model.GetVertice(face.GetVertexIndex(0) - 1), model, camera);
	glm::vec3 point2 = TransVector(model.GetVertice(face.GetVertexIndex(2) - 1), model, camera) - TransVector(model.GetVertice(face.GetVertexIndex(0) - 1), model, camera);

	glm::vec3 normal = glm::normalize(cross(point1, point2)) * glm::vec3(60);

	std::cout << "normals- Again" << std::endl;
	DrawLine(TransVector(model.GetFaceCenter(face), model, camera), normal + TransVector(model.GetFaceCenter(face), model, camera), model.FaceNormalsColor);
...
void Renderer::DrawNormalsVertics(const MeshModel& model, const Camera& camera)
{
	...
	for (int i = 0; i < model.GetVerticesCount(); i++) {
		std::cout << glm::to_string(TransVector(model.GetNormal(i), model, camera)) << std::endl;
		std::cout << glm::to_string(model.GetNormal(i)) << std::endl;
		std::cout << glm::to_string(TransVector(model.GetVertice(i), model, camera)) << std::endl;
		DrawLine(TransVector(model.GetNormal(i), model, camera), TransVector(model.GetVertice(i), model, camera), model.VerticsNormalsColor);
	}
}
